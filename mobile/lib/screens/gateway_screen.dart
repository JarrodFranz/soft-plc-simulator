...
class _GatewayScreenState extends State<GatewayScreen> {
  ...
  void didUpdateWidget(covariant GatewayScreen oldWidget) {
    ...
    if (widget.currentProject.id != oldWidget.currentProject.id) {
      ...
      final newS7Port = widget.currentProject.protocols!.s7!.port.toString();
      if (_s7PortController.text != newS7Port) {
        _s7PortController.value = TextEditingValue(
          text: newS7Port,
  ...
  String _s7StatusLabel(S7HostStatus s) {
    switch (s) {
      case S7HostStatus.stopped:
        return 'Stopped';
      case S7HostStatus.running:
        return 'Running';
      case S7HostStatus.error:
        return 'Error';
    }
  }

  Color _s7StatusColor(S7HostStatus s) {
    switch (s) {
      case S7HostStatus.stopped:
        return Colors.grey;
      case S7HostStatus.running:
  ...
  Future<void> _startS7Hosting() async {
    await widget.s7Host.start(() => widget.currentProject);
  }
  ...
  /// Shared guard for all nine protocol-map "Regenerate" buttons — the policy
  /// lives in `ui/delete_feedback.dart`'s "destructive replace" section.
  ///
  /// [label] names the map ('Modbus register'), [currentCount] is how many rows
  /// are about to be thrown away, and [apply] performs the regeneration and
  /// returns the new row count. A non-empty map is confirmed first; an empty one
  /// goes straight through (its own empty state tells the user to press this).
  /// Either way the result is announced with an UNDO action.
  Future<void> _regenerateMap({
    required String label,
    required int currentCount,
    required int Function() apply,
  }) async {
    if (currentCount > 0) {
      final ok = await confirmDestructiveReplace(
        context,
        title: 'Regenerate $label map?',
        message: 'This rebuilds the map from the project tags and replaces all '
            '$currentCount current ${currentCount == 1 ? 'entry' : 'entries'}, '
            'including any you edited by hand.\n\nYou can undo it afterwards.',
      );
      if (!ok) return;
      if (!mounted) return;
    }
    ...
    showUndoSnackBar(
      context,
      'Regenerated $label map from project tags — '
      '$count ${count == 1 ? 'entry' : 'entries'}',
    );
  }

  Future<void> _autoGenerateMap() => _regenerateMap(
        label: 'OPC UA node',
        currentCount: widget.currentProject.protocols?.opcua?.map.nodes.length ?? 0,
        apply: () {
          setState(() {
            _ensureProtocols();
          ...
          return widget.currentProject.protocols!.opcua!.map.nodes.length;
        },
      );

  Future<void> _autoGenerateModbusMap() => _regenerateMap(
        label: 'Modbus register',
        currentCount: widget.currentProject.protocols?.modbus?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureModbus();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.modbus!.map.entries.length;
        },
      );
  ...
  void _deleteModbusEntry(ModbusMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'Modbus mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateDnpMap() => _regenerateMap(
        label: 'DNP3 point',
        currentCount: widget.currentProject.protocols?.dnp3?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureDnp();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.dnp3!.map.entries.length;
        },
      );
  ...
  void _deleteDnpEntry(DnpMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'DNP3 mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateMqttMap() => _regenerateMap(
        label: 'MQTT tag',
        currentCount: widget.currentProject.protocols?.mqtt?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureMqtt();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.mqtt!.map.entries.length;
        },
      );
  ...
  void _deleteMqttEntry(MqttMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'MQTT mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateEnipMap() => _regenerateMap(
        label: 'EtherNet/IP tag',
        currentCount: widget.currentProject.protocols?.ethernetIp?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureEnip();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.ethernetIp!.map.entries.length;
        },
      );
  ...
  void _deleteEnipEntry(CipMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'EtherNet/IP mapping for "${entry.tagName}"');
  }

  Future<void> _autoGenerateS7Map() => _regenerateMap(
        label: 'S7 area',
        currentCount: widget.currentProject.protocols?.s7?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureS7();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.s7!.map.entries.length;
        },
      );

  /// Appends a default entry to the S7 area map — mirrors `_addModbusEntry`.
  /// New entries land in the auto-generation data block at offset 0; the
  /// operator moves them from there.
  void _addS7Entry(List<String> tagOptions) {
    setState(() {
      _ensureS7();
      widget.currentProject.protocols!.s7!.map.entries.add(S7MapEntry(
        tag: tagOptions.isNotEmpty ? tagOptions.first : '',
        area: kS7AreaNameDb,
  ...
  void _deleteS7Entry(S7MapEntry entry) {
    setState(() {
      widget.currentProject.protocols!.s7!.map.entries.remove(entry);
    });
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'S7 mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateFinsMap() => _regenerateMap(
        label: 'FINS area',
        currentCount: widget.currentProject.protocols?.fins?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureFins();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.fins!.map.entries.length;
        },
      );
  ...
  void _deleteFinsEntry(FinsMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'FINS mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateSlmpMap() => _regenerateMap(
        label: 'SLMP device',
        currentCount: widget.currentProject.protocols?.slmp?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureSlmp();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.slmp!.map.entries.length;
        },
      );
  ...
  void _deleteSlmpEntry(SlmpMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'SLMP mapping for "${entry.tag}"');
  }

  Future<void> _autoGenerateBacnetMap() => _regenerateMap(
        label: 'BACnet object',
        currentCount: widget.currentProject.protocols?.bacnet?.map.entries.length ?? 0,
        apply: () {
          setState(() {
            _ensureBacnet();
          ...
          widget.onProjectUpdated();
          return widget.currentProject.protocols!.bacnet!.map.entries.length;
        },
      );
  ...
  void _deleteBacnetEntry(BacnetMapEntry entry) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'BACnet mapping for "${entry.tag}"');
  }
  ...
  /// yet — mirrors `_ensureEnip`: mutate in memory only, no
  ...
  void _setS7Enabled(bool enabled) {
    setState(() {
      _ensureS7();
      widget.currentProject.protocols!.s7!.enabled = enabled;
    });
    if (!enabled && widget.s7Host.status != S7HostStatus.stopped) {
      unawaited(widget.s7Host.stop());
    }
    widget.onProjectUpdated();
  }

  void _setS7Port(String value) {
    final parsed = int.tryParse(value.trim());
    if (parsed == null || parsed < 0 || parsed > 65535) {
      return; // ignore invalid input; keep the last-valid persisted port
    }
  ...
  void _deleteOpcuaCredential(OpcUaUserCredential credential) {
    ...
    widget.onProjectUpdated();
    showDeleteUndoSnackBar(context, 'OPC UA credential "${credential.username}"');
  }
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed:
                                  (running || !widget.hostingSupported) ? null : _startHosting,
        ...
        children: [
          Row(
            children: [
              ...
              TextButton.icon(
                key: const Key('regen_opcua_button'),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_modbus_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
                      ...
                      Wrap(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _dnpStatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
                          ...
                          Text(
                            ...
                            style: TextStyle(
                                color: Colors.cyanAccent.withValues(alpha: 0.85), fontSize: 12),
                          ),
                        ],
                      ),
                      const SizedBox(height: 12),
                      ...
                      Flex(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed:
                                  (running || !widget.hostingSupported) ? null : _startDnpHosting,
                              child: const Text('Start hosting'),
                            ),
                          ),
                          SizedBox(width: isCompact ? 0 : 12, height: isCompact ? 8 : 0),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_dnp3_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _enipStatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
  ...
  /// The S7comm protocol card: header + enable switch, hosting controls
  /// (Start/Stop, port, status, endpoint), and (when enabled) the area-map
  /// editor. Mirrors `_buildEnipCard`, with two additions S7comm needs:
  ///
  ///  * a PRIVILEGED-PORT note whenever the configured port is below 1024
  ///    (S7comm's standard port, 102, is), since binding it on Linux/macOS
  ///    needs elevation; and
  ///  * a prominent BIND-FAILURE block, so a start that did not get its
  ///    socket reads as a failure rather than as a card that merely failed to
  ///    turn green.
  Widget _buildS7Card(BuildContext context, List<String> tagOptions) {
    final s7 = widget.currentProject.protocols!.s7!;
    final isCompact = context.isCompact;
          ...
          children: [
            Row(
              children: [
                ...
                Switch(
                  key: const Key('s7_enable_switch'),
                  value: s7.enabled,
                  onChanged: _setS7Enabled,
                ),
              ],
            ),
            if (!s7.enabled)
              ...
              const SizedBox(height: 12),
              const Text(
                'Addressed by memory area and byte offset, so a driver can '
                'block-read a whole data block in one request.',
                style: TextStyle(color: Colors.white70, fontSize: 11),
              ),
              const SizedBox(height: 12),
              // ── Hosting controls ────────────────────────────────────
              // Scoped to `widget.s7Host` alone (the WS-perf-task-1 pattern
              // every other card here follows): only the status chip, client
              // count, port field's enabled state and Start/Stop buttons are
              // driven by host notifies. The virtualized area map below is
              // never rebuilt by a host `notifyListeners()`.
              ListenableBuilder(
                listenable: widget.s7Host,
                builder: (context, _) {
                  final status = widget.s7Host.status;
                  final running = status == S7HostStatus.running;
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Wrap(
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _s7StatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(
                          isDense: true,
                          labelText: 'Port',
                          helperText: 'Default: 102',
                          filled: true,
                          fillColor: Color(0xFF0F172A),
                          border: OutlineInputBorder(),
                        ),
                        onChanged: _setS7Port,
                      ),
                      // The privileged-port caveat. Shown whenever the port is
                      // below 1024 — which the DEFAULT (102) is — because on
                      // Linux and macOS binding it needs elevation, and the
                      // fix (edit this field) is right above the note.
                      if (s7.port < 1024) ...[
                        const SizedBox(height: 8),
                        Text(
                          'Port ${s7.port} is a privileged port (below 1024). On Linux and '
                          'macOS, binding it requires elevated privileges — without them '
                          'Start hosting will fail with a permission error, shown below. '
                      ...
                      Flex(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed:
                                  (running || !widget.hostingSupported) ? null : _startS7Hosting,
                              child: const Text('Start hosting'),
                            ),
                          ),
                          SizedBox(width: isCompact ? 0 : 12, height: isCompact ? 8 : 0),
                          SizedBox(
                            width: isCompact ? double.infinity : null,
                            child: OutlinedButton(
                              onPressed: running ? _stopS7Hosting : null,
                              child: const Text('Stop hosting'),
                            ),
                          ),
                        ],
                      ...
                      if (!widget.hostingSupported) ...[
                        const SizedBox(height: 8),
                        Text(
                          'Hosting runs the S7comm server inside the app on a TCP socket, '
                          'which web browsers do not allow. Run the desktop '
                      ...
                      ] else if (widget.s7Host.lastError != null) ...[
                        const SizedBox(height: 8),
                        Text(
              ...
              const SizedBox(height: 12),
              _s7MapEditorCard(context, s7.map, tagOptions),
            ],
          ],
        ),
      ),
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _finsStatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
                      ...
                      Flex(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed:
                                  (running || !widget.hostingSupported) ? null : _startFinsHosting,
                              child: const Text('Start hosting'),
                            ),
                          ),
                          SizedBox(width: isCompact ? 0 : 12, height: isCompact ? 8 : 0),
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _slmpStatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
                      ...
                      Flex(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed:
                                  (running || !widget.hostingSupported) ? null : _startSlmpHosting,
                              child: const Text('Start hosting'),
                            ),
                          ),
                          SizedBox(width: isCompact ? 0 : 12, height: isCompact ? 8 : 0),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_slmp_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final deviceDropdown = DropdownButtonFormField<String>(
            ...
            style: const TextStyle(fontSize: 12, color: Colors.white),
            dropdownColor: const Color(0xFF1E293B),
            items: kSlmpDeviceNames.map((d) => DropdownMenuItem(value: d, child: Text(d))).toList(),
            onChanged: (v) {
              if (v == null) {
                return;
              }
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_bacnet_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Bytes no entry covers read as zero and discard writes. A tag only '
            'partly inside a written range is left unchanged.',
            style: TextStyle(color: Colors.grey, fontSize: 11),
          ),
          const SizedBox(height: 8),
          if (map.entries.isEmpty)
            ...
            _virtualizedMapList<S7MapEntry>(
              map.entries,
              (e) => e.tag,
              (e) => _s7Row(e, tagOptions),
              listKey: const Key('s7_map_list'),
            ),
        ],
      ),
    );
  ...
  Widget _s7Row(S7MapEntry entry, List<String> tagOptions) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 6),
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final areaDropdown = DropdownButtonFormField<String>(
            ...
            style: const TextStyle(fontSize: 12, color: Colors.white),
            dropdownColor: const Color(0xFF1E293B),
            items: kS7AreaNames.map((a) => DropdownMenuItem(value: a, child: Text(a))).toList(),
            onChanged: (v) {
              if (v == null) {
                return;
              }
          ...
          // Only the data-block area carries a block number; the merker and
          // process-image areas are flat address spaces.
          final dbField = TextFormField(
            initialValue: entry.dbNumber.toString(),
            enabled: entry.area == kS7AreaNameDb,
            keyboardType: TextInputType.number,
            style: const TextStyle(fontSize: 12, color: Colors.white),
            decoration: const InputDecoration(isDense: true, labelText: 'DB'),
            onChanged: (v) {
              final parsed = int.tryParse(v.trim());
              if (parsed == null || parsed < 0 || parsed > 65535) {
                return;
              }
              entry.dbNumber = parsed;
              widget.onProjectUpdated();
            },
          );
          final byteField = TextFormField(
            initialValue: entry.byteOffset.toString(),
            keyboardType: TextInputType.number,
            style: const TextStyle(fontSize: 12, color: Colors.white),
            decoration: const InputDecoration(isDense: true, labelText: 'Byte'),
            onChanged: (v) {
              final parsed = int.tryParse(v.trim());
              if (parsed == null || parsed < 0) {
                return;
              }
              entry.byteOffset = parsed;
              widget.onProjectUpdated();
            },
          );
          // Only meaningful for BOOL tags; 0 for everything else.
          final bitField = TextFormField(
            initialValue: entry.bitOffset.toString(),
            keyboardType: TextInputType.number,
            style: const TextStyle(fontSize: 12, color: Colors.white),
          ...
          if (isCompact) {
            return Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: dbField),
                    const SizedBox(width: 4),
                    Expanded(child: byteField),
                    const SizedBox(width: 4),
                    Expanded(child: bitField),
                  ],
                ),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_fins_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final areaDropdown = DropdownButtonFormField<String>(
            ...
            style: const TextStyle(fontSize: 12, color: Colors.white),
            dropdownColor: const Color(0xFF1E293B),
            items: kFinsAreaNames.map((a) => DropdownMenuItem(value: a, child: Text(a))).toList(),
            onChanged: (v) {
              if (v == null) {
                return;
              }
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_enip_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height: 10,
                                decoration: BoxDecoration(
                                    color: _mqttStatusColor(status), shape: BoxShape.circle),
                              ),
                              const SizedBox(width: 8),
                              Text(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: DropdownButtonFormField<String>(
                              ...
                              decoration:
                                  const InputDecoration(isDense: true, labelText: 'Payload format'),
                              style: const TextStyle(fontSize: 12, color: Colors.white),
                              dropdownColor: const Color(0xFF1E293B),
                              items: const [
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: TextFormField(
                              ...
                              style: const TextStyle(fontSize: 12, color: Colors.white),
                              decoration:
                                  const InputDecoration(isDense: true, labelText: 'Heartbeat (s)'),
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: TextFormField(
                              ...
                              style: const TextStyle(fontSize: 12, color: Colors.white),
                              decoration: const InputDecoration(
                                  isDense: true, labelText: 'Publish interval (ms)'),
                          ...
                          SizedBox(
                            ...
                            child: TextFormField(
                              ...
                              style: const TextStyle(fontSize: 12, color: Colors.white),
                              decoration: const InputDecoration(
                                  isDense: true, labelText: 'Deadband (analog)'),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    key: const Key('regen_mqtt_button'),
                    icon: const Icon(Icons.autorenew, size: 16, color: Colors.cyanAccent),
                    label: const Text('Regenerate', style: TextStyle(color: Colors.cyanAccent)),
        ...
        children: [
          for (var i = 0; i < widget.tabs.length; i++)
            ChoiceChip(
              ...
              side: BorderSide(
                color: controller.index == i ? Colors.cyanAccent : Colors.white24,
              ),
