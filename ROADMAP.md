...
- **Deliverables**:
  ...
  - SCADA client interoperability verified with Kepware/UAExpert (manual confirmation on top of the automated client probe). ⏳ **pending the user's live test** — the user must confirm a strict client (Ignition/UAExpert/Kepware) connects to the Basic256Sha256/SignAndEncrypt endpoint with a regenerated cert before this is marked done.
- **Status**: 🔄 **ACTIVE — in-app pure-Dart OPC UA server v1 + v2 subscriptions + v3 Basic256Sha256 security & user auth shipped, strict-client conformance gaps (KeyUsage/EKU, serverSignature, ActivateSession signature verify) closed and regression-guarded by the live Rust E2E (ADR-010; single app hosts); external-strict-client confirmation (Ignition/UAExpert/Kepware) remains pending the user's live test**
