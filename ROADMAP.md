...
- **Deliverables**:
  ...
  - **Per-project Outbound Protocols configuration** ✅ — a per-project `ProtocolSettings` (gateway endpoint + a config slot per outbound protocol) surfaced in an **Outbound Protocols** section where each protocol has an enable/disable toggle (multiple may be enabled) and is configured in place. OPC UA is the one served today; the model + section are structured so Modbus/MQTT/DNP3 slot in with real config when each server ships (no dead placeholders). The client exposes tags only for enabled protocols; configuration persists per-project (additive `protocols` field, back-compat migration of the earlier `opcua_map`).
