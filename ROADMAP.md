...
- **Deliverables**:
  - **In-app pure-Dart OPC UA server v1** 🔄 — `opc.tcp` binary protocol implemented in Dart inside the app (no FFI, no companion): Security `None` + anonymous, Hello/Ack, secure channel (incl. token renewal), sessions, GetEndpoints, Browse/Read/Write over the per-project exposed-tag map, reading the live tag DB in-process and writing force-aware. Verified by codec fixtures (cross-checked against the Rust reference), socketless state-machine tests, and the Rust OPC UA client probe — all runnable in-environment. Runs on Android/desktop/iOS (iOS: foreground-only serving, an OS constraint).
  - Subscriptions/MonitoredItems (live SCADA monitoring) — v2. ⏳
...
- **Status**: 🔄 **ACTIVE — pivoted to in-app hosting (ADR-010); pure-Dart OPC UA server v1 in progress (WS19)**
