...
| `functionBlock`-kind POU with a graphical (`LD`/`FBD`/`SFC`) body | **Not imported for PLCopen input; L5X FBD AOIs are the one exception** | `mapImportedFbs` skips a PLCopen FB-kind POU whose body isn't plain text or neutral ladder - not even a stub `FbDefinition` is created; a warning names the POU. The L5X dialect gate is the exception: an AOI whose `Logic` routine is FBD imports as a real FBD-bodied `FbDefinition` (see [rockwell-l5x.md](./rockwell-l5x.md) §5) - a PLCopen FBD `functionBlock` POU still keeps the same "not imported" warning pending PLCopen-specific validation. `SFC`-kind POUs are unaffected either way. See [../iec61131/custom-function-blocks.md](../iec61131/custom-function-blocks.md). |
