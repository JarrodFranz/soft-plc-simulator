#!/usr/bin/env bash
# S7comm E2E machine-proof: starts the in-app S7comm host (the Dart fixture
# host, mobile/tool/s7_host_probe.dart) on a non-default port, waits for it to
# report READY, then runs a REAL third-party S7comm client -- the Python
# `python-snap7` library (a wrapper around the snap7 C library), driven by
# tool/py/s7_probe.py -- against it.
#
# WHAT THIS PROVES that the Dart unit suite cannot: every other S7comm test in
...
#   COTP Connection Request -> Connection Confirm, then S7 Setup Communication
#   -> its Ack_Data reply (snap7 performs both inside connect()), then asserts
...
# never imports the app's ChangeNotifier-based host service and prints
# `READY ...` once bound; a third-party client probe exiting 0 with a
# `... PROBE PASS` line or non-zero naming the failing step; the client library
# pinned EXACTLY in `tool/py/requirements.txt`; venv create/reuse, quiet
# install, READY handshake, unconditional teardown via `trap`, and the probe's
# exit code propagated verbatim. The venv, its packages, and any `__pycache__`
# are git-ignored -- nothing downloaded here is ever committed.
#
# Usage: tool/s7_e2e.sh   (run from anywhere; bash/Git-Bash)
#
...
# silently fails to find it and the real dart.exe keeps running (and keeps the
# port bound). This script instead finds the REAL Windows PID that is
# listening on the target port via `netstat -ano` and kills that (with `//T` to
# take any of its own children too), falling back to the bash job PID on
# non-Windows. Mirrors `tool/enip_e2e.sh`.
set -u

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
# Non-default: S7comm's real port is 102, which is privileged on most systems
# and would collide with anything already hosting it.
PORT=10102
PY_DIR="${REPO_ROOT}/tool/py"
VENV_DIR="${PY_DIR}/.venv"
DART_LOG="$(mktemp -t s7-e2e-dart-host.XXXXXX 2>/dev/null || echo /tmp/s7-e2e-dart-host.log)"
DART_JOB_PID=""
...
if [ -z "${PY_BIN}" ]; then
  log "FAILED: no Python interpreter on PATH (need Python 3.8+ for python-snap7)."
  exit 1
fi
...
(
  cd "${REPO_ROOT}/mobile" && exec dart run tool/s7_host_probe.dart "${PORT}"
) >"${DART_LOG}" 2>&1 &
DART_JOB_PID=$!
...
for _ in $(seq 1 180); do
  if grep -q "READY s7-tcp://" "${DART_LOG}" 2>/dev/null; then
    READY=1
    break
  fi
  sleep 0.5
...
log "running the python-snap7 client probe against 127.0.0.1:${PORT}..."
# Every request is a loopback round trip and the probe bounds each snap7
# socket operation at 5s itself; 120s is a generous outer bound that still
# cannot wedge CI if the probe somehow blocks outside a socket call.
timeout 120 "${VENV_PY}" "${PY_DIR}/s7_probe.py" 127.0.0.1 "${PORT}"
PROBE_EXIT=$?
