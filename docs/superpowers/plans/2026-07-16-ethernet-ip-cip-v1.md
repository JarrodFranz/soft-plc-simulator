...
>
> **Post-implementation note (added after the real-client E2E found the connection-id direction bug):** `byTargetId` above was renamed to `byConnectionId` — it resolves a connection by the **O->T** id (the id this target allocates and the id an incoming `SendUnitData` is addressed to), not by any "target id" in the abstract. Workstreams that reuse this plan as a template (S7comm, FINS, SLMP, BACnet) should name any equivalent lookup after the concrete direction it keys on, not a generic "target"/"originator" label — the real-client probe caught this exact ambiguity twice in this workstream (once in the Forward Open reply's allocated ids, once in the `SendUnitData` reply's Connected Address item).
