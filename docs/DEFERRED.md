...
| FBD custom-FB call routing | near-term | `mapImportedFbs`/the LD translator route ST-bodied FB calls today; the FBD import translator doesn't exist yet (see "FBD & SFC graphical translators" below), so an FBD `<block>` calling a custom FB still stubs like any other unsupported FBD block. Routing FBD FB calls to instances is the natural follow-up once that translator ships. |
...
| `<expression>`-dialect FB call operands | later | `plcopen_parser` reads a graph node's operand from a descendant `<variable>` element only; a call site whose `<inVariable>`/`<outVariable>` binds its operand via `<expression>` yields a null source, so the FB call rung stubs (`unresolved-operand`). Pre-existing parser convention governing ALL LD operand resolution (not FB-specific); add an `<expression>` fallback in the parser. (IMPORT-FB whole-branch review.) |
